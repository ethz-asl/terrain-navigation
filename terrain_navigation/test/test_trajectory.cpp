...
TEST(TrajectoryTest, getArcCenter) {
  ...
  Eigen::Vector2d arc_center = Trajectory::getArcCenter(segment_start, segment_start_tangent, 1.0);

  ASSERT_TRUE(arc_center.isApprox(Eigen::Vector2d(0.0, 1.0)));
  segment_start_tangent << -1.0, 0.0;
  Eigen::Vector2d arc_center2 = Trajectory::getArcCenter(segment_start, segment_start_tangent, -1.0);
...
TEST(TrajectoryTest, getArcProgress2) {
  ...
  double theta;
  Eigen::Vector2d arc_center = Trajectory::getArcCenter(segment_start, segment_start_tangent, 1.0);
  std::cout << "[getArcProgress2] arc_center: " << arc_center.transpose() << std::endl;
  std::cout << "[getArcProgress2] expected_center: " << expected_center.transpose() << std::endl;
...
TEST(TrajectoryTest, getArcProgress3) {
  // Test close to full circle arc segments
  Eigen::Vector2d segment_start(1.0, 0.0);
  Eigen::Vector2d segment_start_tangent(0.0, -1.0);
  double end_theta = -1.75 * M_PI;
...
TEST(TrajectoryTest, getArcProgress4) {
  // Test close to full circle arc segments
  Eigen::Vector2d segment_start(1.0, 0.0);
  Eigen::Vector2d segment_start_tangent(0.0, -1.0);
